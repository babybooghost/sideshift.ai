...
async function* anthropicStream({ apiKey, model, system, history, userText, imageDataUrl, maxTokens }) {
  ...
  const res = await fetch(ANTHROPIC_URL, {
    ...
    body: JSON.stringify({ model, max_tokens: maxTokens || 1500, stream: true, ...(system ? { system } : {}), messages })
  });
  if (!res.ok) {
